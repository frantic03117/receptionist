...
const AcceptedReservations = () => {
    ...
    const [time_period, setTimePeriod] = React.useState('today');
    ...
    const columns = [
        ...
        },
        {
            ...
            renderCell: (params) => (
                <>
                    {
                        ['Pending', 'Processing'].includes(params.row.status) ? (
                            <>
                                ...
                                <div className="grid grid-cols-12">
                                    ...
                                    <div className="col-span-4">
                                        <div className="flex gap-1 *:rounded items-center *:text-xs *:px-2 *:py-1 ">
                                            ...
                                            <button onClick={() => handleTimePeriod('0')} className={`${time_period == "0" ? 'bg-[var(--primary)] text-white' : 'bg-gray-300 text-black'}`}>All</button>
                                            {
                                                ['past', 'today', 'upcoming'].map(itm => (
                                                    <>
                                                        <button onClick={() => handleTimePeriod(itm)} className={`${time_period == itm ? 'bg-[var(--primary)] text-white' : 'bg-gray-300 text-black'} `}>{itm}</button>
